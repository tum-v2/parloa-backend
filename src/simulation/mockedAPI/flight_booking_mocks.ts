...
export function auth(booking_number: string, last_name: string) {
  if (booking_number.toUpperCase() == 'PARL0A' && last_name.toUpperCase() in VALID_NAMES) {
...
function check_availability(booking_number: string, new_date: string, auth_token: string) {
  if (booking_number.toUpperCase() == 'PARL0A' && auth_token == VALID_TOKEN)
...
function change_flight_date(booking_number: string, new_date: string, auth_token: string): boolean {
  if (
